...
def operand_check(element,current_equation,temp_number_list):
    ...
    if (element not in PARENTHESIS) and Trans.b == (len(current_equation)):
...
def run(current_equation):
    ...
    for element in current_equation:
        ...
        temp_number_list = operand_check(element,current_equation,temp_number_list)
    ...
    appended_operands = empty_stack(appended_operands)
    actual_stack.reversee()
...
def eval_postfix(postfix_tokens):
    ...
    while postfix_tokens.size != 0: # Appendar tal tills det kommer en "charachter" och utför då operationen på de två senaste talen, pga polish notation.
        ...
        if c == "+":
            temp_calc_list.append(temp_calc_list.pop() + temp_calc_list.pop())
        elif c == "-":
            a, b  = temp_calc_list.pop(), temp_calc_list.pop()
            temp_calc_list.append(b-a)
        elif c == "*":
            temp_calc_list.append(temp_calc_list.pop() * temp_calc_list.pop())
        elif c == "/":
            a, b = temp_calc_list.pop(), temp_calc_list.pop()
            if a == 0:
                return DIV_BY_ZERO
            temp_calc_list.append(float(b / a))
        elif c == "^":
            x, y = temp_calc_list.pop(), temp_calc_list.pop()
            #temp_calc_list.append(x**y)
            g=y
            for i in range(x-1):
                g=g*y
            temp_calc_list.append(g)
        else:
